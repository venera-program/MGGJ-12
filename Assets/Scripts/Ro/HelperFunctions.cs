...
public class HelperFunctions
{
    ...
    public static bool IsOnScreen(Vector2 position, float topBorder = 0f, float bottomBorder = 0f, float leftBorder = 0f, float rightBorder = 0f)
    { // can modify this to take in borders
        if (!main)
        {
            main = Camera.main;
        }
    ...
    public static ScreenBorders CloseToBorder(Vector2 position)
    {
        ...
        if (y < corners[2].y)
        { // || (y - corners[2].y < y - corners[0].y)
            return ScreenBorders.Bottom;
        }
        if (x < corners[0].x)
        { //  || (x - corners[0].x > x - corners[1].x)
            return ScreenBorders.Left;
        }
        if (x > corners[1].x)
        { // || (x - corners[1].x < x - corners[0].x)
