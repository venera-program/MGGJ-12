...
public class Enemy_Spawner : MonoBehaviour
{
    ...
    private void Start(){
        LevelClock.Instance.tickTock.AddListener(SpawnEnemies);
        LevelClock.Instance.endOfClock.AddListener(EndProcess);
    }
    ...
    public void SpawnEnemies(int tick){
        if(canSpawn){
            if (Instance.spawnInfos.ContainsKey(tick)){
            foreach (var enemyInfo in spawnInfos[tick]){
